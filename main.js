...
async function init(ev) {
  ...
  const options = await getOptions() // changed
...
async function changed(ev) {
  ...
  const you = ev.target.parentElement.textContent // grab you
  ...
  const they = await getThemProblem(you) // give you to the problem
